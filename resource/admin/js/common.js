...
function ShowDeleteDialog(url, message) {
	var msg = '';
	if(message == undefined) {
		msg = lang['delete_confirm'];
	} else {
		msg = message;
	}
	parent.dialog({
...
$.fn.ruleSingleCheckbox = function() {
	var singleCheckbox = function(checkObj) {
		if(checkObj.prop("disabled") == true) {
			var switchery = new Switchery(checkObj.get(0), {
				disabled: true
			});
			return;
		}
		if(checkObj.hasClass('blue')) {
			var switchery = new Switchery(checkObj.get(0), {
				color: '#41b7f1'
			});
			return;
		}
		if(checkObj.hasClass('pink')) {
			var switchery = new Switchery(checkObj.get(0), {
				color: '#ff7791'
			});
			return;
		}
		if(checkObj.hasClass('teal')) {
			var switchery = new Switchery(checkObj.get(0), {
				color: '#3cc8ad'
			});
			return;
		}
		if(checkObj.hasClass('red')) {
			var switchery = new Switchery(checkObj.get(0), {
				color: '#db5554'
			});
			return;
		}
		if(checkObj.hasClass('yellow')) {
			var switchery = new Switchery(checkObj.get(0), {
				color: '#fec200',
