...
class dao_DealLoanType extends Dao
{
    ...
    public function getColumns()
    {
        ...
        , 'types'//贷款类型：0学生贷,1信用贷,2抵押贷,3普惠贷
        , 'is_quota'//额度限制  0否 1是
        , 'content'//类型简介
        ...
        , 'zm_point_limit'//芝麻信用分限制：低于设定分值的将被自动拒绝
        );
    }
    ...
    {
        $data = \Core::cache()->get('deal_loan_type_all_data');
        if ($data) {
            return $data;
        }
        ...
        $typeFields = $this->getColumns();
        array_shift($typeFields);
        $typeFields = array_map(function ($str) {
            return 'dlte.' . $str;
        }, $typeFields);
        ...
        $data = $this->getDb()
            ...
            ->from($this->getTable(), 'dlt')
            ->join(['deal_loan_type_extern' => 'dlte'], 'dlt.id = dlte.loan_type_id', 'left')
            ->where([])
            ->execute()
            ->key('id')
            ->rows();
        \Core::cache()->set('deal_loan_type_all_data', $data);
        return $data;
    }
