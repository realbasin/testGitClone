...
class dao_UserSta extends Dao
{
    ...
    public function getUserIdList($userIdArr)
    {
        $data = $this->getDb()
            ->select('user_id')
            ->from($this->getTable())
            ->where(['user_id' => $userIdArr, 'success_deal_count' => 1])
            ->execute()
            ->rows();
    ...
    public function getUserIdList2($userIdArr)
    {
        $data = $this->getDb()
            ->select('user_id')
            ->from($this->getTable())
