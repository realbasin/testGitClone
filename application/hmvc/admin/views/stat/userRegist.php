 	...
 	<div  id="flexitable" class="flexitable">
      ...
      <thead>
        <tr>
          ...
          <th width="200" style="width: 200px;" align="center">注册人数</th>
          <th></th>
        </tr>
      </thead>
      <tbody>
...
function btnPress(name, grid) {
    if (name == 'csv') {
        window.location.href = '<?php echo adminUrl('stat_platform','userRegist_export',array('datestart'=>$datestart,'dateend'=>$dateend));?>';
    }
};
...
$('#btnsearch').on('click',function(){
	...
	var url='<?php echo adminUrl('stat_platform','userRegist');?>';
...
function initSearch(){
	...
	var dateend=$('#dateend').val();
	$.ajax({
		url:'<?php echo adminUrl('stat_platform','userRegist_json');?>',
  		type:'get',
  		data:{
  			datestart:datestart,
  			dateend:dateend
...
function fillCharts(data){
	...
	$.each(data, function(key,val) {
		...
		usercountArr[key]=parseInt(val.usercount);
	});
