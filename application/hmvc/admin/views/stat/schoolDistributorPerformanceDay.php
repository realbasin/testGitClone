...
$(function(){
	$("#flexitable").flexigrid({
        url: "<?php echo adminUrl('stat_distributor','schoolDistributorPerformance_day_json');?>&id=<?php echo $id;?>&"+$("#form1").serialize(),
        colModel : [
        	{display: '时间', name : 'sta_date', width : 100, sortable : true, align: 'center'},
            {display: '新增客户', name : 'user_new', width : 80, sortable : true, align: 'center'}, 
			{display: '借款客户', name : 'user_borrow', width : 100, sortable : true, align : 'center'},
			{display: '借款总额', name : 'borrow_amount', width : 100, sortable : true, align: 'center'},
			{display: '还款总额', name : 'repay_amount', width : 100, sortable : true, align: 'center'},
			{display: '首借还款', name : 'repay_fisrt', width : 80, sortable : true, align: 'left'},
			{display: '续借还款', name : 'repay_more', width : 80, sortable : true, align: 'left'}
            ],
        buttons : [
...
//导出
function flexExport(){
	 var url= '<?php echo adminUrl('stat_distributor','schoolDistributorPerformance_day_export',array('id'=>$id,'user_name'=>$userName));?>'+'&'+$("#form1").serialize();
...
$('#btnsearch').on('click',function(){
	$("#flexitable").flexOptions({url: '<?php echo adminUrl('stat_distributor','schoolDistributorPerformance_day_json',array('id'=>$id));?>'+'&'+$("#form1").serialize(),query:'',qtype:''}).flexReload();
});
