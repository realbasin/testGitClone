...
<div class="location">
  ...
  <i class="arrow"></i>
  <span>下级用户列表 - <?php echo $userName;?></span>
</div>
...
$(function(){
	$("#flexitable").flexigrid({
        url: "<?php echo adminUrl('stat_distributor','schoolDistributorPerformance_subordinate_json');?>&id=<?php echo $id;?>&"+$("#form1").serialize(),
        colModel : [
        	{display: 'id', name : 'id', width : 100, sortable : true, align: 'center'},
            {display: '用户名称', name : 'user_name', width : 100, sortable : true, align: 'center'}, 
...
//导出
function flexExport(){
	 var url= '<?php echo adminUrl('stat_distributor','schoolDistributorPerformance_subordinate_export',array('id'=>$id,'user_name'=>$userName));?>'+'&'+$("#form1").serialize();
...
$('#btnsearch').on('click',function(){
	$("#flexitable").flexOptions({url: '<?php echo adminUrl('stat_distributor','schoolDistributorPerformance_subordinate_json',array('id'=>$id));?>'+'&'+$("#form1").serialize(),query:'',qtype:''}).flexReload();
});
