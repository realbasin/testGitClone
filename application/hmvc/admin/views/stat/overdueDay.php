...
$(function(){
	$("#flexitable").flexigrid({
        url: '<?php echo adminUrl('stat_borrow','overdueDay_json');?>'+'&'+$("#form1").serialize(),
        colModel : [
            {display: '日期', name : 'date_time', width : 100, sortable : true, align: 'center'}, 
			{display: '新增笔数', name : 'add_count_deal', width : 70, sortable : false, align : 'center'},
			{display: '新增期数', name : 'add_over_times', width : 70, sortable : false, align: 'center'},
			{display: '新增管理费', name : 'add_sum_repay_manage_money', width : 100, sortable : false, align: 'center'},
			{display: '新增本息', name : 'add_sum_repay_money', width : 100, sortable : false, align: 'center'},
			{display: '新增本金', name : 'add_sum_self_money', width : 100, sortable : false, align: 'center'},
			{display: '减少笔数', name : 'cut_count_deal', width : 70, sortable : false, align : 'center'},
			{display: '减少期数', name : 'cut_over_times', width : 70, sortable : false, align: 'center'},
			{display: '减少管理费', name : 'cut_sum_repay_manage_money', width : 100, sortable : false, align: 'center'},
			{display: '减少本息', name : 'cut_sum_repay_money', width : 100, sortable : false, align: 'center'},
			{display: '减少本金', name : 'cut_sum_self_money', width : 100, sortable : false, align: 'center'}
            ],
        buttons : [
...
function flexPress(name, grid) {
    if (name == 'csv') {
        window.location.href = '<?php echo adminUrl('stat_borrow','overdueDay_export',array('datestart'=>$datestart?$datestart:date('Y-m-d',strtotime('-30 day')),'dateend'=>$dateend?$dateend:date('Y-m-d',time())));?>';
    }
};
...
$('#btnsearch').on('click',function(){
	...
	$("#flexitable").flexOptions({url: '<?php echo adminUrl('stat_borrow','overdueDay_json');?>&datestart='+datestart+'&dateend='+dateend,query:'',qtype:''}).flexReload();
});
