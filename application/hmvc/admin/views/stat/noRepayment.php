 	...
 	<div  id="flexitable" class="flexitable">
      ...
      <thead>
        <tr>
          ...
          <th width="100" style="width: 100px;" align="center">日期</th>
          <th width="120" style="width: 120px;" align="center">待还总额</th>
          <th width="120" style="width: 120px;" align="center">待还本金</th>
          <th width="120" style="width: 120px;" align="center">待还利息</th>
          <th width="100" style="width: 100px;" align="center">待还人次</th>
          <th></th>
        </tr>
      </thead>
      <tbody>
...
$('#btnsearch').on('click',function(){
	...
	var url='<?php echo adminUrl('stat_borrow','noRepayment');?>';
...
function initSearch(){
	...
	var dateend=$('#dateend').val();
	$.ajax({
		url:'<?php echo adminUrl('stat_borrow','noRepayment_json');?>',
  		type:'get',
  		data:{
  			datestart:datestart,
  			dateend:dateend
...
function fillFlexTable(data){
	...
	$.each(data, function(key,val) {
		jsonhtml+='{"id":"'+key+'",';
		jsonhtml+='"cell":[';
		jsonhtml+='"'+val.nopayment_date+'",';
		jsonhtml+='"'+val.nopayment_amount+'",';
		jsonhtml+='"'+val.nopayment_capital+'",';
		jsonhtml+='"'+val.nopayment_interest+'",';
		jsonhtml+='"'+val.nopayment_number+'",';
		jsonhtml+='""]},';
	});
...
function fillCharts(data){
	var datestart=$('#datestart').val();
	var dateend=$('#dateend').val();
	var nopayment_date_Arr=[];
	var nopayment_amount_Arr=[];
	var nopayment_capital_Arr=[];
	var nopayment_interest_Arr=[];
	var nopayment_number_Arr=[];
	
	$.each(data, function(key,val) {
		nopayment_date_Arr[key]=val.nopayment_date;
		nopayment_amount_Arr[key]=parseFloat(val.nopayment_amount);
		nopayment_capital_Arr[key]=parseInt(val.nopayment_capital);
		nopayment_interest_Arr[key]=parseFloat(val.nopayment_interest);
		nopayment_number_Arr[key]=parseInt(val.nopayment_number);
	});
    ...
    series: [{
        name: '待还金额',
        data: nopayment_amount_Arr
    },{
        name: '待还本金',
        data: nopayment_capital_Arr
    },{
        name: '待还利息',
        data: nopayment_interest_Arr
    },{
        name: '待款人次',
        data: nopayment_number_Arr
    }]
});
}
