...
class Http {
	...
	private $ch, $last_url,
		...
		$referer, $error = array('code' => 0, 'msg' => ''), $sleep = 0

	;

	/**
	 * 设置每次请求之后sleep的时间，单位毫秒
	 * @param type $microSeconds
	 * @return \Http
	 */
	...
	/**
	 * 设置cookie文件存储路径
	 * @param type $cookie_path cookie文件路径
	 * @return \Http
	 */
	public function setCookieFilePath($cookie_path = null) {
		if (empty($cookie_path)) {
			$cookie_path = $this->getTempDir() . '/' . 'http_cookie_' . md5(uniqid('', TRUE));
			$functionName = 'clean_' . md5(uniqid('', TRUE));
	...
	/**
	 * 设置请求超时时间，单位毫秒
	 * @param type $timeoutMilliSeconds
	 * @return \Http
	 */
	public function setTimeout($timeoutMilliSeconds) {
		curl_setopt($this->ch, CURLOPT_CONNECTTIMEOUT_MS, (int) $timeoutMilliSeconds);
		return $this;
	}
