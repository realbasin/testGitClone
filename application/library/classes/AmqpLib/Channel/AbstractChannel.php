<?php
namespace AmqpLib\Channel;

use AmqpLib\Connection\AbstractConnection;
use AmqpLib\Exception\AMQPOutOfBoundsException;
use AmqpLib\Exception\AMQPOutOfRangeException;
use AmqpLib\Exception\AMQPRuntimeException;
use AmqpLib\Helper\DebugHelper;
use AmqpLib\Helper\Protocol\MethodMap080;
use AmqpLib\Helper\Protocol\MethodMap091;
use AmqpLib\Helper\Protocol\Protocol080;
use AmqpLib\Helper\Protocol\Protocol091;
use AmqpLib\Helper\Protocol\Wait080;
use AmqpLib\Helper\Protocol\Wait091;
...
abstract class AbstractChannel
{
    ...
    /** @var \AmqpLib\Helper\DebugHelper */
    protected $debug;
    ...
    /** @var \AmqpLib\Helper\Protocol\Protocol080|\AmqpLib\Helper\Protocol\Protocol091 */
    protected $protocolWriter;

    /** @var \AmqpLib\Helper\Protocol\Wait080|\AmqpLib\Helper\Protocol\Wait091 */
    protected $waitHelper;

    /** @var \AmqpLib\Helper\Protocol\MethodMap080|\AmqpLib\Helper\Protocol\MethodMap091 */
    protected $methodMap;

    /** @var string */
    protected $channel_id;

    /** @var \AmqpLib\Wire\AMQPReader */
     ...
     * @throws \AmqpLib\Exception\AMQPRuntimeException
     */
    public function __construct(AbstractConnection $connection, $channel_id)
    {
        ...
        switch ($this->protocolVersion) {
            case self::PROTOCOL_091:
                self::$PROTOCOL_CONSTANTS_CLASS = 'AmqpLib\Wire\Constants091';
            ...
            case self::PROTOCOL_080:
                self::$PROTOCOL_CONSTANTS_CLASS = 'AmqpLib\Wire\Constants080';
                $c = self::$PROTOCOL_CONSTANTS_CLASS;
                $this->debug = new DebugHelper($c);
                $this->amqp_protocol_header = $c::$AMQP_PROTOCOL_HEADER;
     ...
     * @param \AmqpLib\Wire\AMQPWriter|string $args
     */
     ...
     * @param array $method_sig
     * @param \AmqpLib\Wire\AMQPWriter|string $args
     * @param \AmqpLib\Wire\AMQPWriter $pkt
     * @return \AmqpLib\Wire\AMQPWriter
     */
     ...
     * @param int $timeout
     * @throws \AmqpLib\Exception\AMQPOutOfBoundsException
     * @throws \AmqpLib\Exception\AMQPRuntimeException
     * @return mixed
     */
