<?php
/**
 * Tsign sdk php e签宝快捷签 SDK PHP版实现类
 *
 * User: wanglf
 * Date: 2016/12/14
 *
...
use Tsign\constants\ErrorConstant;
use Tsign\constants\SignType;
use Tsign\constants\LicenseType;
use Tsign\result\AddAccountResult;
use Tsign\result\AddEventCertResult;
use Tsign\result\AddTemplateResult;
use Tsign\result\FileSignResult;
use Tsign\result\GetSignDetailResult;
use Tsign\result\Result;
use Tsign\result\TextSignResult;
use Tsign\result\VerifyPdfResult;
use Tsign\core\JavaComm;
use Tsign\core\HttpUtils;
use Tsign\core\Util;
use Tsign\core\Recorder;
use Tsign\core\Upload;
...
class eSign
{
     ...
     * 版本号
     */
    const VERSION = '2.0.000';

    /**
     * 发送渠道
     */
    const CHANNEL = 'phptechsdk2.0.000';
    ...
    public function __construct($app_env='')
    {
        ...
        if ($app_env == 'production') {
            self::$_CONFIG['open_api_url'] = self::$_CONFIG['open_api_url_www'];
            self::$_CONFIG['project_id'] = self::$_CONFIG['project_id_www'];
            self::$_CONFIG['project_secret'] = self::$_CONFIG['project_secret_www'];
        } else {
            self::$_CONFIG['open_api_url'] = self::$_CONFIG['open_api_url_test'];
            self::$_CONFIG['project_id'] = self::$_CONFIG['project_id_test'];
            self::$_CONFIG['project_secret'] = self::$_CONFIG['project_secret_test'];
        }
    ...
    private function needInit()
    {
        ...
        //设备是否相同
        $sameEquip = substr($this->recorder->read('equipId'), 0, 17) == substr(Util::getMacAddr(), 0, 17);
        if (!$sameEquip) {
            return true;
        }
        return false;
    ...
    private function getAuthUrlList()
    {
        ...
        //检测错误是否发生
        $res = HttpUtils::request()->noSignHttpPost(self::$_CONFIG['open_api_url'], $keysArr, false, true);
        $ret = new Result($res);
        $res = $ret->getData();
     ...
     * @return array|mixed
     */
    public function upatePersonAccount($accountId, $modifyArray = array())
    {
        if (empty($accountId)) {
                                    ...
                                    $qText = '')
    {
        ...
        if (empty($templateType)
            || !in_array($templateType, PersonTemplateType::getArray())
            && !in_array($templateType, OrganizeTemplateType::getArray())
        ) {
                                  ...
                                  $stream = false)
    {
        ...
        if ($stream === true) {
            $res = $this->javaComm->userSignPDFStream($accountId, $signFile, $signPos, $signType, $sealData, $mobile, $code);
        } else {
            $res = $this->javaComm->userSignPDF($accountId, $signFile, $signPos, $signType, $sealData, $mobile, $code);
        }
        $result = new FileSignResult($res);
        return $result->getData();
    }
    ...
    /**
     * 文档保全
     * @param $docFilePath string 要保全的文件
     * @param $docName string 保存的文件名称
     * @param $signServerIdd string 签署记录ids, 多个id用 逗号“,”分割
     * @return array
     */
    public function saveSignedFile($docFilePath, $docName, $signServerIds)
    {
        $docFilePath = Util::encodePath($docFilePath);
        if (!is_file($docFilePath)) {
            return ErrorConstant::$SIGNED_FILE_NOT_EXISTS;
        }
        return $this->_saveSignedFile($docFilePath, $docName, $signServerIds);
    }

    /**
     * 文档保全 文件流
     * @param $fileStream 要保全的文件
     * @param $docName 保存的文件名称
     * @param $signServerIdd 签署记录ids, 多个id用 逗号“,”分割
     * @return array
     */
    public function saveSignedFileStream($fileStream, $docName, $signServerIds)
    {
        if (empty($fileStream)) {
            return ErrorConstant::$SIGNED_FILE_NOT_EXISTS;
        }
        return $this->_saveSignedFile($fileStream, $docName, $signServerIds);
    }
     ...
     * @return array
     */
    private function _saveSignedFile($file, $docName, $signServerIds)
    {
        if (empty($docName)) {
            return ErrorConstant::$DOC_NAME_NULLPOINTER;
        }
        $signServerIds = explode(',', $signServerIds);
        if (empty($signServerIds)) {
            return ErrorConstant::$SIGNER_NOT_EXISTS;
        }
        //文件后缀名
        $fileExt = strtolower(substr($file, strrpos($file, '.')));
        if (strtolower(substr($docName, strrpos($docName, '.'))) !== $fileExt) {
            $docName .= $fileExt;
        }
        //文件直传
        $res = $this->uploadFile($file, $docName);
        if (empty($res['fileKey'])) {
            return $res;
        }
        ...
        $keysArr = array(
            'docName' => $docName,
            'ossKey' => $res['fileKey'],
            'signlogIds' => $signServerIds
        );
        $res = $this->withSignHttpPost(self::TECH_SAVE_FILE_WITH_SINGID_URL, $keysArr);
        $result = new Result($res);
        return $result->getData();
    }
    ...
    private function uploadFile($file, $filename)
    {
        ...
        //获取文件上传授权地址 //上传文件
        $res = $this->getUploadSignUrl($fileMd5, $filename);
        if (!empty($res['url'])) {
     ...
     * @return array|mixed
     */
    private function getUploadSignUrl($fileMd5, $filename)
    {
        $keysArr = array(
            'fileName' => $filename,
            'md5' => $fileMd5,
    ...
    {
        if (empty($docId)) {
            return ErrorConstant::$DOCID_NULLPOINTER;
        }
        $keysArr = array(
            'docId' => $docId
        );
        $res = $this->withSignHttpPost(self::TECH_GET_DOC_URL, $keysArr);
        $result = new Result($res);
        return $result->getData();
    }
