 ...
 * 验证码组成类型，有三种：1.number（纯数字） 2.letter（纯字母） 3.both（字母和数字）
...
class Captcha {
	...
	private $width = 80, $height =30, $codenum = 4
		, $checkcode     //产生的验证码
		, $checkimage    //验证码图片 
		, $session_flag = 'captcha_code' //存到session中的索引
		, $font_path
		, $codeMode = 'both'; //验证码组成：1.number 2.letter 3.both
	...
	function __construct() {
		\Core::sessionStart();
		$this->font_path = $this->getTempDir() . '/' . md5('captchattf') . '.ttf';
	...
	/**
	 * 设置验证码组成
	 * @param type $codeMode 验证码组成类型，参数值有三种：1.number（纯数字） 2.letter（纯字母） 3.both（数字和数字）
	 * @return \Dz_Captcha
	 * @throws Dz_exception_500
	 */
	public function setCodeMode($codeMode) {
		if (!in_array($codeMode, array('number', 'letter', 'both'))) {
			throw new \Xs_Exception_500('error type [ ' . $codeMode . ' ] , should be one of [ number,letter,both ]');
		}
